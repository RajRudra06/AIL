 ...
 */
export function runCheckpoint1(gitRepos: string[], workspacePath: string, analysisDir: string): CommitHistoryResult {
    const commits: CommitInfo[] = [];

    for (const repoPath of gitRepos) {
        try {
            // Get last 200 commits with stats per repo
            const raw = execSync(
            ...
            console.warn(`Git log failed in repo ${repoPath}. Details: ${err.message || err}`);
        }
    }
